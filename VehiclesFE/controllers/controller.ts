...
function submitCar(){
    ...
    let content_ruedas = (<HTMLInputElement>document.getElementById("ruedas"));
    let resultado = (<HTMLInputElement>document.getElementById('resultado'));
    let btn_createCar = (<HTMLInputElement>document.getElementById('create-car'));

    let num_errors:number= 0;
    ...
    if(num_errors > 0){
        ...
        (<HTMLInputElement>document.getElementById('carInfo')).innerHTML = 'Matrícula: '+plate+ ' Marca: ' + brand+ ' Color: ' + color;
    }
}
...
function submitWeels(){
    ...
    for (let i =1; i <=4; i++) {
        let mr = (<HTMLInputElement>document.getElementById('mr_'+i)).value;
        let dr = Number((<HTMLInputElement>document.getElementById('dr_'+i)).value);
...
function printWheels(){
    for (let i = 0; i < car.wheels.length; i++) {
        (<HTMLInputElement>document.getElementById('wheel'+i)).innerHTML = 'Marca Rueda '+(i+1)+':'+car.wheels[i].brand;
        (<HTMLInputElement>document.getElementById('diameter'+i)).innerHTML = 'Diametro Rueda '+(i+1)+':'+car.wheels[i].diameter;
    }
}